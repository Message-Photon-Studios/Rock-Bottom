...
public class ColorSpellImpact : SpellImpact
{
    ...
    public override void Impact(Collider2D other)
    {
        ...
        {
            GameObject test = Physics2D.Raycast(transform.position, other.transform.position-transform.position,1000, ~LayerMask.GetMask("Spell", "Player", "Ignore Raycast")).collider.gameObject;
